    ...
    <div className="relative min-h-screen">
      ...
      <div className="relative p-6 lg:p-8 max-w-4xl mx-auto">
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <Badge className="mb-4 bg-[#00E599]/10 text-[#00E599] border-[#00E599]/30 font-mono text-[10px]"><PawPrint className="w-3 h-3 mr-1.5" /> ROADMAP</Badge>
          <h1 className="text-4xl lg:text-5xl font-semibold tracking-tight mb-2" style={{ fontFamily: 'var(--font-display)' }}>
            New Pet <span className="text-[#00E599]">Care Plan</span>
          </h1>
        ...
        <div className="flex items-center justify-between gap-4 mb-8 flex-wrap">
          <div className="flex gap-2">
            {([['puppy', 'Puppy', Dog], ['kitten', 'Kitten', Cat]] as const).map(([k, label, Icon]) => (
              <button key={k} onClick={() => setKind(k)} aria-pressed={kind === k}
                className={`flex items-center gap-2 px-4 py-2 rounded-xl border text-sm transition-all ${kind === k ? 'bg-[#00E599]/15 border-[#00E599]/40 text-[#00E599]' : 'bg-white/[0.03] border-white/8 text-zinc-400 hover:border-white/20'}`}>
          ...
          <div className="glass-card rounded-xl px-4 py-2.5 flex items-center gap-3">
            ...
            <span className="text-2xl font-semibold tabular-nums text-[#00E599]" style={{ fontFamily: 'var(--font-display)' }}>{pct}%</span>
            <div className="w-24 h-1.5 bg-white/[0.06] rounded-full overflow-hidden"><motion.div animate={{ width: `${pct}%` }} className="h-full bg-[#00E599]" /></div>
          </div>
        </div>
              ...
              <motion.div key={wk.week} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: wi * 0.08 }} className="glass-card rounded-2xl p-6">
                ...
                <div className="space-y-2">
                  {wk.steps.map((step, si) => {
                    ...
                    return (
                      <button key={id} onClick={() => toggle(id)} className={`w-full flex items-center gap-3 p-3 rounded-xl text-left transition-all ${isDone ? 'bg-[#00E599]/[0.07]' : 'hover:bg-white/[0.03]'}`}>
                        {isDone ? <div className="w-5 h-5 rounded-md bg-[#00E599] flex items-center justify-center shrink-0"><Check className="w-3.5 h-3.5 text-[#04130D]" /></div> : <Circle className="w-5 h-5 text-zinc-600 shrink-0" />}
