...
export default function PredictPage() {
  ...
  const riskLevel =
    risk >= 80 ? { label: 'CRITICAL', color: '#FF5E3A', desc: 'Severe burnout imminent. Clinical intervention strongly recommended within 30 days.' } :
    risk >= 60 ? { label: 'HIGH', color: '#FBBF24', desc: 'Significant risk. Consider workplace change or formal mental health support.' } :
    risk >= 40 ? { label: 'MEDIUM', color: '#8B5CF6', desc: 'Manageable but watch for escalation. Establish boundaries now.' } :
                 { label: 'LOW', color: '#10D9B8', desc: 'Healthy baseline. Maintain current habits and monitor signals.' }
  ...
  const benchmark = [
    ...
    { name: `${industry.name} avg`, value: Math.round(industry.baseline * 1.05), color: '#FF8A65' },
    ...
    <div className="min-h-screen bg-[#08090B] text-zinc-100 relative">
      ...
      <div className="relative max-w-5xl mx-auto px-6 py-12">
        ...
        <div className="flex items-center gap-2 mb-8">
          {[0, 1, 2, 3].map(i => (
            <button
              ...
              className={`h-1.5 rounded-full transition-all ${i === step ? 'w-12 bg-[#FF5E3A]' : i < step ? 'w-6 bg-[#FF5E3A]/40' : 'w-6 bg-white/10'}`}
              aria-label={`Step ${i + 1}`}
            />
          ))}
        ...
        <motion.div
          ...
          className="p-8 rounded-3xl border border-white/8 bg-[#0F1012]/80 backdrop-blur-md mb-6 min-h-[420px]"
        >
          {step === 0 && (
            <>
              <div className="flex items-center gap-3 mb-2">
                <Building2 className="w-5 h-5 text-[#FF5E3A]" />
                  ...
                  {ROLES.map(r => (
                    <button
                      ...
                      className={`px-3 py-2.5 rounded-xl border text-sm transition-all ${
                        role === r ? 'bg-[#FF5E3A]/15 border-[#FF5E3A]/40 text-[#FF5E3A]' : 'bg-white/[0.03] border-white/8 text-zinc-300 hover:border-white/20'
                      }`}
                    >
                      {r}
                    </button>
          ...
          {step === 1 && (
            <>
              <div className="flex items-center gap-3 mb-2">
                <Clock className="w-5 h-5 text-[#FF5E3A]" />
              ...
              <div className="mb-10">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="font-mono text-2xl text-[#FF5E3A] font-bold">{hours}h</span>
                </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="font-mono text-2xl text-[#FF5E3A] font-bold">{tenure}m</span>
                </div>
          ...
          {step === 2 && (
            <>
              <div className="flex items-center gap-3 mb-2">
                <AlertTriangle className="w-5 h-5 text-[#FF5E3A]" />
                  ...
                  return (
                    <button
                      ...
                      className={`p-4 text-left rounded-xl border transition-all flex items-center gap-3 ${
                        active ? 'bg-[#FF5E3A]/15 border-[#FF5E3A]/40' : 'bg-white/[0.03] border-white/8 hover:border-white/20'
                      }`}
                    >
                      <div className={`w-5 h-5 rounded-md border flex items-center justify-center shrink-0 ${active ? 'bg-[#FF5E3A] border-[#FF5E3A]' : 'border-white/20'}`}>
          ...
          {step === 3 && (
            <>
              <div className="flex items-center gap-3 mb-6">
                <Target className="w-5 h-5 text-[#FF5E3A]" />
                <h2 className="text-2xl font-bold">Your projection</h2>
              </div>

              <div className="grid lg:grid-cols-2 gap-6 mb-6">
                {/* Score card */}
                <div className="p-6 rounded-2xl border bg-gradient-to-br from-[#16171A] to-[#0F1012]"
                ...
                >
                  ...
                  <div className="mt-5 h-2 bg-white/[0.06] rounded-full overflow-hidden">
                    <motion.div
                      ...
                      style={{ background: `linear-gradient(90deg, #FBBF24, ${riskLevel.color})` }}
                    />
                  </div>
                </div>

                {/* Benchmark */}
                <div className="p-6 rounded-2xl border border-white/8 bg-[#0F1012]">
                  ...
                  <ResponsiveContainer width="100%" height={180}>
                    <BarChart data={benchmark} layout="vertical">
                      ...
                      <Tooltip cursor={{ fill: 'rgba(255,255,255,0.04)' }} contentStyle={{ background: '#16171A', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10 }} />
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          {step < 3 ? (
            <Button onClick={next} className="bg-gradient-to-r from-[#FF5E3A] to-[#FF8A65] hover:from-[#EA4520] text-white border-0 gap-2">
              Continue <ArrowRight className="w-4 h-4" />
            </Button>
          ) : (
            <Link href="/resources">
              <Button className="bg-gradient-to-r from-[#FF5E3A] to-[#FF8A65] hover:from-[#EA4520] text-white border-0 gap-2">
