      ...
      <div className="relative p-6 lg:p-8 max-w-2xl mx-auto">
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <h1 className="text-4xl lg:text-5xl font-semibold tracking-tight mb-2" style={{ fontFamily: 'var(--font-display)' }}>
            Add a <span className="text-[#00E599]">Pet</span>
          </h1>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="glass-card rounded-3xl p-6 space-y-6">
          ...
          <div className="flex items-center gap-4">
            <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-[#00E599] to-[#14E5C8] flex items-center justify-center relative">
              <SpeciesIcon className="w-9 h-9 text-[#04130D]" />
              <button className="absolute -bottom-1 -right-1 w-7 h-7 rounded-lg bg-[#0E1316] border border-white/10 flex items-center justify-center" aria-label="Add photo"><Camera className="w-3.5 h-3.5" /></button>
            </div>
            <div className="flex-1">
              <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider block mb-2">Name</label>
              <Input value={form.name} onChange={e => set('name', e.target.value)} placeholder="e.g. Biscuit" className="bg-[#070A0C] border-white/10 focus:border-[#00E599]/40" />
            </div>
          </div>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
              {SPECIES.map(s => (
                <button key={s.key} onClick={() => set('species', s.key)} aria-pressed={form.species === s.key}
                  className={`flex flex-col items-center gap-1.5 py-3 rounded-xl border text-xs transition-all ${form.species === s.key ? 'bg-[#00E599]/15 border-[#00E599]/40 text-[#00E599]' : 'bg-white/[0.03] border-white/8 text-zinc-400 hover:border-white/20'}`}>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <Input value={form.breed} onChange={e => set('breed', e.target.value)} placeholder="e.g. Beagle" className="bg-[#070A0C] border-white/10 focus:border-[#00E599]/40" />
            </div>
            <div>
              <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider block mb-2">Age</label>
              <Input value={form.age} onChange={e => set('age', e.target.value)} placeholder="e.g. 3 years" className="bg-[#070A0C] border-white/10 focus:border-[#00E599]/40" />
            </div>
            <div>
              <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider block mb-2">Weight (kg)</label>
              <Input type="number" value={form.weight} onChange={e => set('weight', e.target.value)} placeholder="e.g. 12.4" className="bg-[#070A0C] border-white/10 focus:border-[#00E599]/40" />
            </div>
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {['male', 'female'].map(s => (
                  <button key={s} onClick={() => set('sex', s)} aria-pressed={form.sex === s}
                    className={`px-3 py-2 rounded-xl border text-sm capitalize transition-all ${form.sex === s ? 'bg-[#00E599]/15 border-[#00E599]/40 text-[#00E599]' : 'bg-white/[0.03] border-white/8 text-zinc-400 hover:border-white/20'}`}>
                    {s}
                  </button>
                ))}
              </div>
