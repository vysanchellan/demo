...
export default function AssessmentPage() {
  ...
  if (result) {
    ...
    return (
      <div className="min-h-screen bg-[#050708] flex items-center justify-center px-6 py-12">
        ...
        >
          <div className="p-8 bg-[#0A0D0F] border border-white/8 rounded-3xl text-center">
            <h1 className="text-4xl font-black mb-2" style={{ fontFamily: 'var(--font-display)' }}>YOUR RESULTS</h1>
            <p className="text-[#93A29E] mb-10">Based on your 16 responses</p>
            ...
            <div className="relative w-48 h-48 mx-auto mb-10">
              ...
              <div className="absolute inset-6 rounded-full flex flex-col items-center justify-center" style={{ background: '#050708', boxShadow: `0 0 60px ${color}30` }}>
            ...
            <div className="grid grid-cols-3 gap-4 mb-10">
              {[
                { label: 'Exhaustion', value: result.exhaustion, color: '#00E599' },
                { label: 'Cynicism', value: result.cynicism, color: '#5EEAD4' },
                { label: 'Low Efficacy', value: result.efficacy, color: '#14E5C8' },
              ].map(s => (
                <div key={s.label} className="p-4 bg-[#050708] rounded-xl border border-white/8">
                  <div className="text-2xl font-black mb-1" style={{ color: s.color, fontFamily: 'var(--font-display)' }}>{s.value}</div>
                  <div className="text-[#93A29E] text-xs">{s.label}</div>
                  <div className="mt-2 h-1 bg-[#12161A] rounded-full overflow-hidden">
                    <motion.div
                      initial={{ width: 0 }}
  ...
  return (
    <div className="min-h-screen bg-[#050708] flex items-center justify-center px-6 py-12 relative overflow-hidden">
      <div className="absolute inset-0 bg-mesh-soft" />

      <div className="relative z-10 w-full max-w-2xl">
        {/* Progress */}
        <div className="mb-8">
          <div className="flex items-center justify-between text-sm text-[#93A29E] mb-2">
          ...
          <div className="h-1.5 bg-[#12161A] rounded-full overflow-hidden">
            <motion.div
              animate={{ width: `${((current + 1) / QUESTIONS.length) * 100}%` }}
              transition={{ duration: 0.3 }}
              className="h-full rounded-full bg-gradient-to-r from-[#14E5C8] to-[#00E599]"
            />
          </div>
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="flex items-center gap-2 mb-6">
              <span className="text-xs font-semibold px-2.5 py-1 rounded-full text-[#93A29E] bg-white/5 border border-white/10">
                ...
                <motion.button
                  ...
                  className={`p-3 rounded-xl border text-center transition-all duration-200 ${
                    answers[q.id] === s.value
                      ...
                      : 'bg-[#050708] border-white/10 text-[#93A29E] hover:border-[#00E599]/40 hover:text-white'
                  }`}
                >
